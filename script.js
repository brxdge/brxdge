...
async function loadBlog(){
  ...
  // blogData holds both post types from one feed — split it here rather
  // than at fetch time so both grids always reflect the latest fetch.
  // Case studies render inside the merged #proof section (see
  // renderCaseStudiesGrid()); everything else (postType !== 'case-study',
  // which also covers plain articles with no postType set) is a Blog
  // article and renders in the separate #blog section below
  // (renderArticlesGrid()).
...
// The merged #proof section (Campaign Results & Case Studies) hides
// itself only when BOTH sub-groups have nothing to show — each sub-group
// hides/shows itself independently (renderCampaignsGrid()/
// renderCaseStudiesGrid()), then calls this to reconcile the outer
// section's visibility against whichever sub-groups are currently
// visible. Safe to call from either render function in either order.
function updateProofSectionVisibility(){
  const section = document.getElementById('proof');
  if(!section) return;
  const campaignsSub = document.getElementById('campaignsSubsection');
  const caseStudiesSub = document.getElementById('caseStudiesSubsection');
  const campaignsHidden = !campaignsSub || campaignsSub.style.display === 'none';
  const caseStudiesHidden = !caseStudiesSub || caseStudiesSub.style.display === 'none';
  section.style.display = (campaignsHidden && caseStudiesHidden) ? 'none' : '';
}

// Case-study half of the old mixed blog feed — renders into the
// "Case Studies" sub-group inside the merged #proof section.
function renderCaseStudiesGrid(){
  const sub = document.getElementById('caseStudiesSubsection');
  const grid = document.getElementById('caseStudiesGrid');
  if(!sub || !grid) return;

  const caseStudies = blogData.filter(p => p.postType === 'case-study');
  if(!caseStudies.length){
    sub.style.display = 'none';
  } else {
    sub.style.display = '';
    grid.innerHTML = '';
    caseStudies.forEach(p => grid.appendChild(buildBlogCard(p)));
    revealTalentCards(grid);
  }
  updateProofSectionVisibility();
}

// Everything else in the feed (plain articles — postType !== 'case-study')
// — renders into the standalone #blog section, split out of the old
// mixed feed per the client's "add a BLOG section" request.
function renderArticlesGrid(){
  ...
  // No published articles yet — hide the whole section rather than show
  // a placeholder/fake post. Real content only, added via the admin
  // dashboard.
  if(!articles.length){
...
function renderCampaignsGrid(){
  const sub = document.getElementById('campaignsSubsection');
  const grid = document.getElementById('campaignsGrid');
  if(!sub || !grid) return;

  // Same rule as the case-studies sub-group: no real campaigns yet means
  // this sub-group doesn't show, never placeholder brand results. The
  // merged #proof section itself only disappears if Case Studies is also
  // empty — see updateProofSectionVisibility().
...
/* ---------------- SMOOTH SCROLL TO SECTION (used by inline onclick handlers) ----------------
   window.__fpGoToSlideById is set by the FULLPAGE SLIDES block further down
   this file, only once it's actually running (desktop, 2+ slides found,
   no reduced-motion) — when it's present, native scrollIntoView() would do
   nothing useful (the page itself no longer scrolls; sections are moved
   via transform instead), so this routes through the slide controller
   instead. Falls back to the original scrollIntoView for everyone else
   (mobile/touch, reduced-motion, or talent.html's 2-slide case if that
   ever drops below the 2-slide minimum). */
function scrollToSection(id){
  if (window.__fpGoToSlideById && window.__fpGoToSlideById(id)) return;
...
/* ---------------- FULLPAGE SLIDES (client reference: night.co) ----------------
   Turns every top-level `main > section` (plus the footer, as the final
   "slide") into a full-viewport slide, moved via a single translateY on
   #fpWrapper (see the HTML/CSS comments on that element) instead of native
   document scrolling — the same technique night.co uses (there: fullPage.js;
   here: a small purpose-built version, so this doesn't take on that
   library's licensing). One wheel/key/swipe input moves exactly one slide,
   landing on it edge-to-edge, and reverses the same way scrolling back up —
   matching the "scroll down, scroll back up" transition the client pointed
   to on that site.

   Scope, deliberately: DESKTOP ONLY (pointer:fine + hover:hover) and only
   when the visitor hasn't requested reduced motion. Touch/mobile visitors
   and reduced-motion visitors get none of this — no class gets added, the
   CSS block in style.css stays fully inert, and the page scrolls exactly
   as it always did. Locking scroll and hijacking wheel/touch gestures is a
   much rougher experience on a touchscreen (fighting the OS's own
   momentum scrolling) than ​on a desktop trackpad/wheel, so this trades
   consistency for a version that actually feels good on each input type,
   rather than a single implementation that's mediocre on both.

   Sections taller than one viewport (most of them, on this site) get their
   own internal scroll first — the wheel handler only advances to the next/
   previous slide once you've scrolled the current one to its own top/
   bottom edge — so no content is ever clipped or unreachable, exactly how
   fullPage.js itself handles an oversized section.

   Known, accepted trade-off: a few scroll-position-driven effects defined
   earlier in this file (initBackgroundParallax's --bg-sy, updateNavOnScroll's
   .scrolled/.hidden navbar states) are keyed off real `window.scrollY`,
   which stays frozen at 0 once the page itself stops scrolling here — they
   simply go inert (frozen at their initial state) rather than erroring,
   which in the navbar's case is actually the same "always visible, never
   auto-hides" behavior night.co's own nav has. initHeroScrollTransition's
   --hero-scroll fade gets an explicit compatibility hook below instead,
   since leaving the hero fully visible while browsing later slides would
   look wrong. The reveal-on-scroll system (.reveal + sectionObserver) and
   the stat count-up/typewriter triggers all key off IntersectionObserver,
   which reports real intersection regardless of transform-based
   positioning — those needed no changes and keep working as-is.

   One more thing this accounts for: a couple of sections (#proof, #blog)
   hide themselves entirely (inline style.display='none' — see
   updateProofSectionVisibility()/renderArticlesGrid() above) whenever
   there's no campaign/case-study/blog content to show yet. A slide list
   captured once at load would drift out of sync the moment that happens
   (every slide after the hidden one would land half a screen off), so the
   visible slide list is recomputed — via a MutationObserver on each
   section's style/class attributes — any time that changes, not just
   once up front. */
(function initFullpageSlides(){
  ...
  // allSlideEls: every candidate slide, structural and fixed for the life
  // of the page. slideEls: just the currently-visible subset of those,
  // recomputed by refreshSlides() — this is what everything below actually
  // navigates against.
  const allSlideEls = Array.from(main.querySelectorAll(':scope > section'));
  if (footer) allSlideEls.push(footer);
  if (allSlideEls.length < 2) return;
  ...
  // Mirrors setBodyScrollLocked()'s own toggle (see near the top of this
  // file) — that function sets this INLINE style when an overlay/modal
  // opens; html.fp-active's own overflow:hidden lives in the stylesheet
  // (style.css), not inline, so it never trips this check on its own.
  ...
  // Re-applies the transform with no transition — used only when the
  // active slide's INDEX shifted because something else became hidden/
  // shown elsewhere in the list, not because the visitor navigated. The
  // slide itself hasn't moved on screen (it's the same element, still
  // either the one showing or still off-screen), so animating this would
  // just be a spurious jump with no real navigation behind it.
  function setTransformInstant(index){
    wrapper.style.transition = 'none';
    setTransform(index);
    void wrapper.offsetHeight; // force reflow so the transition is really off for this one frame
  ...
  function rebuildDots(){
    ...
    dotsEl.className = 'fp-dots';
    slideEls.forEach((el, i) => {
      const dot = document.createElement('button');
      dot.type = 'button';
      dot.className = 'fp-dot' + (i === currentIndex ? ' active' : '');
      dot.setAttribute('aria-label', 'Go to ' + (el.id || ('section ' + (i + 1))));
  ...
  // Rebuilds slideEls/idToIndex from whichever of allSlideEls are actually
  // visible right now. If the previously-active slide is still visible,
  // stays on that exact element (recalculating its new index and
  // snapping the transform to match, instantly — see setTransformInstant);
  // otherwise clamps to the nearest still-visible slide.
  function refreshSlides(){
    const nextSlideEls = allSlideEls.filter(isVisible);

    // No-op guard: the .reveal/.active scroll-entrance system (and other
    // unrelated class/style churn on these same elements) also trips this
    // observer constantly during ordinary slide navigation. Only actually
    // touch the transform/dots when the visible set genuinely changed —
    // otherwise this would interrupt an in-flight transition, and because
    // it forces `transition:none` for a frame, the transitionend listener
    // below would never fire, permanently stranding isAnimating at true
    // and freezing all further navigation.
    if (nextSlideEls.length === slideEls.length && nextSlideEls.every((el, i) => el === slideEls[i])) return;

    const activeEl = slideEls[currentIndex] || null;
    slideEls = nextSlideEls;
    idToIndex = {};
    slideEls.forEach((el, i) => { if (el.id) idToIndex[el.id] = i; });

    const newIndex = activeEl ? slideEls.indexOf(activeEl) : -1;
    currentIndex = newIndex !== -1 ? newIndex : Math.max(0, Math.min(currentIndex, slideEls.length - 1));
    setTransformInstant(currentIndex);
    rebuildDots();
  }
  ...
  function goToSlide(index){
    ...
    if (el && el.id) history.replaceState(null, '', '#' + el.id);
    document.dispatchEvent(new CustomEvent('fpslidechange', { detail: { index: currentIndex, el } }));
    return true;
  }

  // Bridge used by scrollToSection() above and by the hash-link click
  // handler below — returns false when there's no matching slide so
  // callers can fall back to their own default behavior.
  window.__fpGoToSlideById = function(id){
    if (!(id in idToIndex)) return false;
    goToSlide(idToIndex[id]);
    return true;
  };
  ...
  function onWheel(e){
    ...
    // The FAQ popup is the one overlay-ish panel that doesn't call
    // setBodyScrollLocked (it doesn't need to block page scroll on its
    // own) and it sits fixed outside #fpWrapper — leave wheel input over
    // it alone so its own internal scroll keeps working. Deliberately NOT
    // requiring every wheel target to be inside the wrapper more broadly —
    // that would create a dead zone over the fixed navbar/progress dots,
    // which also sit outside the wrapper but should still drive slides
    // when a visitor's cursor happens to be over them while scrolling.
    const faqPanel = document.getElementById('faqPanel');
    if (faqPanel && faqPanel.contains(e.target)) return;
    ...
    const atBottom = Math.ceil(active.scrollTop + active.clientHeight) >= active.scrollHeight;
    const dir = e.deltaY > 0 ? 1 : -1;

    if (dir > 0 && !atBottom) return; // let the section keep scrolling internally first
    if (dir < 0 && !atTop) return;

    const targetIdx = currentIndex + dir;
    if (targetIdx < 0 || targetIdx >= slideEls.length) return; // already at an edge — nothing to do
  ...
  function onKeydown(e){
    if (overlayOpen()) return;
    // Deliberately NOT requiring focus to be inside the wrapper here — by
    // default (nothing clicked yet) document.activeElement is <body>
    // itself, which isn't "inside" anything, and after using a navbar
    // link (the navbar sits outside the wrapper, fixed on top) focus
    // stays on that link — both are completely normal states a visitor
    // should still be able to keep navigating slides from. Only the FAQ
    // popup's own contents (a real interactive panel outside the wrapper)
    // and actual form fields below are excluded.
    const faqPanel = document.getElementById('faqPanel');
    if (faqPanel && faqPanel.contains(document.activeElement)) return;
    ...
    else if (e.key === 'ArrowUp' || e.key === 'PageUp') { e.preventDefault(); goToSlide(currentIndex - 1); }
    else if (e.key === 'Home') { e.preventDefault(); goToSlide(0); }
    else if (e.key === 'End') { e.preventDefault(); goToSlide(slideEls.length - 1); }
  }
  window.addEventListener('keydown', onKeydown);

  // Same-page hash links (nav / mobile tabbar / footer) — native anchor
  // jump can't do anything useful once the page itself no longer scrolls,
  // so route these through goToSlide() instead. Cross-page links like
  // "talent.html" or "index.html#about" don't match [href^="#"] and are
  // untouched.
  document.addEventListener('click', (e) => {
  ...
  // Keyboard users: if Tab (or a script-driven .focus()) lands on something
  // inside a slide that isn't the active one, bring that slide into view
  // too, so focus never ends up somewhere the visitor can't see.
  document.addEventListener('focusin', (e) => {
    const slide = slideEls.find(el => el.contains(e.target));
    if (slide) {
  ...
  // Compatibility hook for initHeroScrollTransition (top of this file) —
  // see the big comment above this IIFE.
  document.addEventListener('fpslidechange', (e) => {
    const hero = document.querySelector('.hero');
    const heroCopy = hero ? hero.querySelector('.hero-copy') : null;
    if (heroCopy) heroCopy.style.setProperty('--hero-scroll', e.detail.index === 0 ? '0' : '1');
  });

  let resizeTimer = null;
  window.addEventListener('resize', () => {
    clearTimeout(resizeTimer);
    resizeTimer = setTimeout(() => setTransform(currentIndex), 150);
  });

  // Watches for the display:none flips described in the big comment above
  // this IIFE — debounced slightly since updateProofSectionVisibility()
  // etc. can touch a couple of elements back-to-back for what's really one
  // content update. Deliberately watches ONLY the `style` attribute, not
  // `class`: the actual code that hides these sections at runtime
  // (updateProofSectionVisibility(), renderArticlesGrid()) sets an inline
  // `style.display`, but nearly every section also carries the site's
  // pre-existing `.reveal` class, which gets `.active` toggled onto it by
  // its own IntersectionObserver the moment it scrolls into view — i.e. on
  // almost every slide transition. Watching `class` too meant that toggle
  // fired refreshSlides() mid-transition, which forces the transform
  // transition off for a frame (setTransformInstant) and so the
  // `transitionend` listener above never got to fire — permanently
  // stranding isAnimating at true and freezing all further navigation.
  // refreshSlides() itself also no-ops when the visible set didn't
  // actually change, as a second layer of protection.
  let refreshTimer = null;
  const visibilityObserver = new MutationObserver(() => {
    clearTimeout(refreshTimer);
    refreshTimer = setTimeout(refreshSlides, 50);
  });
  allSlideEls.forEach(el => visibilityObserver.observe(el, { attributes: true, attributeFilter: ['style'] }));

  // Land on whichever slide the URL hash points to at load (e.g. a link
  // from talent.html's footer to "index.html#about"), instead of always
  // starting at slide 0.
  slideEls = allSlideEls.filter(isVisible);
  slideEls.forEach((el, i) => { if (el.id) idToIndex[el.id] = i; });
  const initialId = location.hash ? location.hash.slice(1) : '';
  currentIndex = idToIndex.hasOwnProperty(initialId) ? idToIndex[initialId] : 0;
  setTransform(currentIndex);
